...
bool ConfidenceScorer::isValidFallSequence()
{
    ...
    return (stage1_score >= 5) &&   // Minimum free fall detected
           (stage2_score >= 4) &&   // Minimum impact detected (lowered for light falls)
           (getTotalScore() >= 22); // Overall minimum threshold (lowered for light falls)
}
...
uint8_t ConfidenceScorer::calculateDurationScore(float duration_ms)
{
    ...
    if (duration_ms >= 100.0f)
        return 5; // Brief drop
    if (duration_ms >= 60.0f)
        return 3; // Very brief drop (light fall)
    return 0;
}
...
uint8_t ConfidenceScorer::calculateMagnitudeScore(float magnitude_g)
{
    ...
    if (magnitude_g <= 0.7f)
        return 3; // Mild weightlessness (light fall)
    return 0;
}
...
uint8_t ConfidenceScorer::calculateImpactScore(float impact_g)
{
    ...
    if (impact_g >= 2.0f)
        return 6; // Light-moderate impact
    if (impact_g >= 1.3f)
        return 4; // Light impact
    return 0;
}
...
uint8_t ConfidenceScorer::calculateAngularScore(float angular_velocity_dps)
{
    ...
    if (angular_velocity_dps >= 100.0f)
        return 4; // Light rotation
    return 0;
}
...
uint8_t ConfidenceScorer::calculateInactivityScore(float duration_ms)
{
    ...
    if (duration_ms >= 1400.0f)
        return 4; // Very brief stillness (light fall)
    return 0;
}

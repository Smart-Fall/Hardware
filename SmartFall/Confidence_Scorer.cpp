...
void ConfidenceScorer::addFSRFilterScore(bool impact_detected, bool strap_secure)
{
    uint8_t fsr_score = 0;
    if (strap_secure)
        fsr_score += 2; // Device attached throughout sequence
    if (impact_detected)
        fsr_score += 3; // Impact spike detected

    filter_breakdown.fsr_filter_score = fsr_score;
    updateFilterScore();
}

void ConfidenceScorer::addPhysiologicalScore(uint16_t current_heart_rate, uint8_t spo2,
                                             uint16_t baseline_heart_rate)
{
    // Heart rate elevation scoring (stress response to fall)
    uint16_t heart_rate_increase = current_heart_rate > baseline_heart_rate ? (current_heart_rate - baseline_heart_rate) : 0;

    if (heart_rate_increase >= 40)
    {
        physiological_breakdown.heart_rate_elevation_score = 8; // Significant elevation
    }
    else if (heart_rate_increase >= 20)
    {
        physiological_breakdown.heart_rate_elevation_score = 5; // Moderate elevation
    }
    else if (heart_rate_increase >= 10)
    {
        physiological_breakdown.heart_rate_elevation_score = 2; // Slight elevation
    }
    else
    {
        physiological_breakdown.heart_rate_elevation_score = 0; // No elevation
    }

    // SpO2 validation (should be normal or slightly reduced due to stress)
    if (spo2 >= 90)
    {
        physiological_breakdown.spo2_validation_score = 5; // Normal oxygenation
    }
    else if (spo2 >= 85)
    {
        physiological_breakdown.spo2_validation_score = 2; // Slightly reduced (stress)
    }
    else
    {
        physiological_breakdown.spo2_validation_score = 0; // Critical low oxygen (concern)
    }

    // Add physiological scores to filter score (physiological response validates mechanical fall)
    filter_score += physiological_breakdown.heart_rate_elevation_score +
                    physiological_breakdown.spo2_validation_score;
    capScore(filter_score, 15); // Increase max filter score to accommodate physiological

    if (DEBUG_ALGORITHM_STEPS)
    {
        Serial.print("Physiological Score: HR: ");
        Serial.print(physiological_breakdown.heart_rate_elevation_score);
        Serial.print(" (");
        Serial.print(current_heart_rate);
        Serial.print(" bpm, baseline: ");
        Serial.print(baseline_heart_rate);
        Serial.print("), SpO2: ");
        Serial.print(physiological_breakdown.spo2_validation_score);
        Serial.print(" (");
        Serial.print(spo2);
        Serial.println("%)");
    }
...
{
    if (duration_ms >= 500.0f)
        return 15; // Extended fall
    if (duration_ms >= 200.0f)
        return 10; // Typical fall
...
{
    if (magnitude_g <= 0.1f)
        return 10; // True free fall
    if (magnitude_g <= 0.3f)
        return 8; // Significant weightlessness
    if (magnitude_g <= 0.5f)
        return 5; // Partial weightlessness
    return 0;
}
...
{
    if (impact_g >= 6.0f)
        return 15; // Severe impact
    if (impact_g >= 4.0f)
        return 12; // Significant impact
    if (impact_g >= 3.0f)
        return 8; // Moderate impact
    return 0;
}
...
{
    if (timing_ms <= 500.0f)
        return 5; // Immediate impact
    if (timing_ms <= 1000.0f)
        return 3; // Delayed impact
    return 0;
}

uint8_t ConfidenceScorer::calculateAngularScore(float angular_velocity_dps)
{
    if (angular_velocity_dps >= 600.0f)
        return 15; // Severe rotation
    if (angular_velocity_dps >= 400.0f)
        return 12; // Significant rotation
    if (angular_velocity_dps >= 250.0f)
        return 8; // Moderate rotation
    return 0;
}

uint8_t ConfidenceScorer::calculateInactivityScore(float duration_ms)
{
    if (duration_ms >= 10000.0f)
        return 15; // Extended incapacitation
    if (duration_ms >= 5000.0f)
        return 12; // Moderate incapacitation
    if (duration_ms >= 2000.0f)
        return 8; // Brief incapacitation
    return 0;
}

uint8_t ConfidenceScorer::calculatePressureScore(float altitude_change_m)
{
    if (altitude_change_m >= 2.0f)
        return 5; // Significant fall height
    if (altitude_change_m >= 1.0f)
        return 3; // Moderate fall height
    if (altitude_change_m >= 0.5f)
        return 2; // Minor fall height
    return 0;
}
...
void ConfidenceScorer::printDetailedAnalysis()
{
    ...
    Serial.print(", FSR: ");
    Serial.print(filter_breakdown.fsr_filter_score);
    Serial.print(", Heart Rate: ");
    Serial.print(physiological_breakdown.heart_rate_elevation_score);
    Serial.print(", SpO2: ");
    Serial.println(physiological_breakdown.spo2_validation_score);

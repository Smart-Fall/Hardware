...
void FallDetector::processSensorData(SensorData_t &data)
{
    ...
    case FALL_STATUS_STAGE2_IMPACT:
        // Stage 3 (rotation) temporarily disabled for testing
        // TODO: Re-enable for real wearable deployment - angular velocity is important for fall classification
        /*
        // Check for rotation during impact
        if (DEBUG_ALGORITHM_STEPS) {
            static uint32_t last_gyro_log = 0;
            if (millis() - last_gyro_log > 100) {
                float angular_mag = calculateAngularMagnitude(data);
                Serial.print("[STAGE2->3] Gyro: ");
                Serial.print(angular_mag, 1);
                Serial.print(" °/s (need: ");
                Serial.print(thresholds.rotation_threshold_dps);
                Serial.println(" °/s)");
                last_gyro_log = millis();
            }
        }
        ...
        // If in Stage 2 for more than 500ms without rotation, skip to Stage 4
        // (Some falls may not have significant rotation)
        if ((millis() - stage2_start_time) > 500) {
            if (DEBUG_ALGORITHM_STEPS) {
                Serial.println("[STAGE2] Timeout - skipping to inactivity check");
            }
        ...
        */
        ...
        // Wait 300ms after impact before checking inactivity
        // This allows the device to settle after the impact
        if ((millis() - stage2_start_time) >= 300)
        {
            if (DEBUG_ALGORITHM_STEPS)
            {
                Serial.println("[STAGE2] Impact complete - proceeding to inactivity check (Stage 3 disabled)");
            }
            current_status = FALL_STATUS_STAGE4_INACTIVITY;
            stage4_start_time = millis();
            inactivity_start_time = stage4_start_time;
    ...
    case FALL_STATUS_STAGE4_INACTIVITY:
        if (checkStage4_Inactivity(data))
        {
            ...
            if ((millis() - inactivity_start_time) >= thresholds.inactivity_threshold_ms)
            {
                ...
                if (DEBUG_ALGORITHM_STEPS)
                {
                    Serial.println("!!! FALL DETECTED: All stages completed !!!");
                }
            }
        }
        else
...
bool FallDetector::checkStage1_FreeFall(SensorData_t &data)
{
    ...
    if (total_accel < thresholds.freefall_threshold_g)
    {
        ...
        return freefall_duration >= 40; // Minimum 40ms free fall
    }
    else
    {
        // Free fall ended
        if (stage1_triggered && freefall_duration >= 40)
        {
            if (DEBUG_ALGORITHM_STEPS)
            {
                Serial.print("[STAGE1] Free fall END - Duration: ");
                Serial.print(freefall_duration);
                Serial.println("ms");
            }
            return true; // Valid free fall phase completed
        }
        if (stage1_triggered && DEBUG_ALGORITHM_STEPS)
        {
            Serial.print("[STAGE1] Free fall TOO SHORT - Duration: ");
            Serial.print(freefall_duration);
            Serial.println("ms (need 40ms)");
        }
...
bool FallDetector::checkStage2_Impact(SensorData_t &data)
{
    ...
    if (total_accel > thresholds.impact_threshold_g)
    {
        if (!stage2_triggered)
        {
            ...
            if (DEBUG_ALGORITHM_STEPS)
            {
                Serial.print("[STAGE2] Impact detected - Accel: ");
                Serial.print(total_accel, 3);
                Serial.print("g, Timing: ");
                Serial.print(impact_timing);
                Serial.println("ms after freefall");
            }
        }
...
bool FallDetector::checkStage3_Rotation(SensorData_t &data)
{
    ...
    if (DEBUG_ALGORITHM_STEPS && angular_mag > 100.0f)
    {
        Serial.print("[STAGE3] Angular velocity: ");
        Serial.print(angular_mag, 1);
...
bool FallDetector::checkStage4_Inactivity(SensorData_t &data)
{
    ...
    // Relaxed thresholds for testing: 0.7-1.3g accel, <100 deg/s gyro
    bool is_inactive = (total_accel > 0.7f && total_accel < 1.3f) &&
                       (angular_mag < 100.0f);

    if (DEBUG_ALGORITHM_STEPS)
    {
        static uint32_t last_inactivity_log = 0;
        if (millis() - last_inactivity_log > 200)
        {
            Serial.print("[STAGE4] Accel: ");
            Serial.print(total_accel, 3);
            Serial.print("g (0.7-1.3), Gyro: ");
            Serial.print(angular_mag, 1);
            Serial.print(" °/s (<100), Inactive: ");
            Serial.print(is_inactive ? "YES" : "NO");
            if (stage4_triggered)
            {
                uint32_t duration = millis() - inactivity_start_time;
